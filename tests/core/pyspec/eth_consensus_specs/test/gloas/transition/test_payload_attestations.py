...
def test_transition_rejects_pre_gloas_payload_attestation(
    ...
    transition_to(spec, state, fork_epoch * spec.SLOTS_PER_EPOCH - 2)
    ...
    # Compute an otherwise valid state root and block signature with the fork-slot restriction disabled.
...
def test_transition_accepts_gloas_payload_attestations(
    ...
    for slot in [state.slot + 1, (fork_epoch + 1) * post_spec.SLOTS_PER_EPOCH]:
