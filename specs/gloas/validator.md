...
`current_epoch <= epoch <= next_epoch`, as PTC committee selection is only
stable within the context of the current and next epoch.
...
def get_ptc_assignment(
    ...
    assert get_current_epoch(state) <= epoch <= next_epoch
