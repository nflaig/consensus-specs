...
def get_parent_payload_status(store: Store, block: BeaconBlock) -> PayloadStatus:
    ...
    parent_block_hash = block.body.signed_execution_payload_commitment.message.parent_block_hash
    message_block_hash = parent.body.signed_execution_payload_commitment.message.block_hash
...
def on_block(store: Store, signed_block: SignedBeaconBlock) -> None:
    ...
    commitment = block.body.signed_execution_payload_commitment.message
    parent_commitment = parent_block.body.signed_execution_payload_commitment.message
...
def validate_merge_block(block: BeaconBlock) -> None:
    ...
    if TERMINAL_BLOCK_HASH != Hash32():
        ...
        assert (
            block.body.signed_execution_payload_commitment.message.parent_block_hash
            == TERMINAL_BLOCK_HASH
        )
        return
